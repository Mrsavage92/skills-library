...
## Skills Library Publishing Constraints

This repository publishes skills to **Skills Library** (clawhub.com) as the distribution registry. The following rules are **non-negotiable**:

1. **cs- prefix for slug conflicts only.** When a skill slug is already taken on Skills Library by another publisher, publish with the `cs-` prefix (e.g., `cs-copywriting`, `cs-seo-audit`). The `cs-` prefix applies **only on the Skills Library registry** — repo folder names, local skill names, and all other tools (Claude Code, Codex, Gemini CLI) remain unchanged.
2. **Never rename repo folders or local skill names** to match Skills Library slugs. The repo is the source of truth.
...
4. **Rate limit: 5 new skills per hour** on Skills Library. Batch publishes must respect this. Use the drip timer (`clawhub-drip.timer`) for bulk operations.
...
6. **Version follows repo versioning.** Skills Library package versions must match the repo release version (currently v2.1.2+).
